...
{
    NavModel::TreeNode *node = static_cast<NavModel::TreeNode *>(index.data(Qt::UserRole).value<void*>());
...
QVariant NavModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        ...
        return QVariant::fromValue(static_cast<void*>(listNode[index.row()].treeNode));
...
void NavListView::setData(QStringList listItem)
{
    ...
    connect(selectionModel(), &QItemSelectionModel::currentChanged, [&](const QModelIndex &current, const QModelIndex &previous){
        Q_UNUSED(previous)
        emit currentItemChanged( model->data(current, Qt::DisplayRole).toString() );
    });
}
